...
def setup_directories():
    for folder_name in ["seed", "seed_split", "real_seeded", "real_seeded_split", "outpainted"]:
...
def generate_seed_images():
    ...
    for vehicle_type in os.listdir(real_path):
        ...
        print(vehicle_type)
        detect.process_folder(input_folder, output_folder, retain_uncropped = True)

def split_dataset(folder_name):
    folder_path = os.path.join(base_path, folder_name)
    folder_split_path = os.path.join(base_path, folder_name+"_split")
    splitter = utils.DatasetSplitter(folder_path, folder_split_path, split_ratios={'train': 0.4, 'val': 0.1, 'test': 0.50})
    splitter.execute()
...
def perform_outpainting():
    ...
    outpaint.generate_outpainted(seed_split_path, outpainted_path)
...
def perform_augmentation():
    ...
    """
    real_seed_split_path = os.path.join(base_path, "real_seeded_split")
...
def run():
    ...
    utils.set_seeds(0)
    add_background(os.path.join(base_path, 'real_seeded_split'))
